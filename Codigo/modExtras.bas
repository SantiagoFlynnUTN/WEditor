...
If SelTexIndex(j) > 0 Then
    ...
    MapData(aX + dX, aY + dy).Graphic(Val(frmMain.cCapas.Text)).index = TexWE(Tex).index(SelTexIndex(j)).Num
...
With MapData(X, Y)
        If .DecorInfo.EstadoDefault > 0 Then 'Asignamos el decorgrh correcto segun estado.
            If .DecorInfo.EstadoDefault <= 4 Then
                If DecorData(.DecorI).DecorGrh(.DecorInfo.EstadoDefault + 1) > 0 Then
                    DECOR_GETGRH_FROMDEFAULT = DecorData(.DecorI).DecorGrh(.DecorInfo.EstadoDefault + 1)
                Else
                    For j = (.DecorInfo.EstadoDefault + 1) To 1 Step -1
